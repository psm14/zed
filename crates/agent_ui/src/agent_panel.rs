...
use acp_thread::{AcpThread, AgentSessionInfo, AgentThreadEntry};
    ...
    cx.observe_new(
        |workspace: &mut Workspace, _window, _cx: &mut Context<Workspace>| {
            workspace
                ...
                .register_action(|workspace, _: &OpenActiveThreadInEditorTab, window, cx| {
                    if let Some(panel) = workspace.panel::<AgentPanel>(cx) {
                        workspace.focus_panel::<AgentPanel>(window, cx);
                        panel.update(cx, |panel, cx| {
                            panel.open_active_thread_in_editor_tab(
...
impl AgentPanel {
    fn serialize(&mut self, cx: &mut App) {
        ...
        let selected_agent = self.selected_agent_for_display(cx);
            ...
            let panel = workspace.update_in(cx, |workspace, window, cx| {
                ...
                if let Some(serialized_panel) = &serialized_panel {
                    panel.update(cx, |panel, cx| {
                        ...
                        panel.ensure_selected_agent_is_visible(cx);
                        cx.notify();
                    });
                }
            ...
            if let Some(thread_info) = serialized_panel.and_then(|p| p.last_active_thread) {
                ...
                if thread_exists {
                    panel.update_in(cx, |panel, window, cx| {
                        if panel.is_agent_visible(&thread_info.agent_type, cx) {
    ...
    pub(crate) fn new(
        ...
        let settings_subscription = cx.observe_global::<SettingsStore>(|this, cx| {
            this.ensure_selected_agent_is_visible(cx);
            cx.notify();
        });
    ...
    fn live_agent_thread_views(&self, cx: &App) -> HashMap<EntityId, Entity<AcpServerView>> {
        ...
        if let Some(current_view) = self.current_thread_view_for_tabs()
            && let ActiveView::AgentThread { thread_view } = current_view
        {
            live_thread_views.insert(Entity::entity_id(thread_view), thread_view.clone());
        }
        for tab in &self.inactive_thread_tabs {
            if let ActiveView::AgentThread { thread_view } = &tab.view {
                live_thread_views.insert(Entity::entity_id(thread_view), thread_view.clone());
            }
        }
    ...
    fn current_thread_view_for_tabs(&self) -> Option<&ActiveView> {
        if Self::is_thread_view(&self.active_view) {
            return Some(&self.active_view);
        }

        match &self.active_view {
            ActiveView::History { .. } | ActiveView::Configuration => self
                .previous_view
                .as_ref()
                .filter(|view| Self::is_thread_view(view)),
            ActiveView::Uninitialized => None,
            ActiveView::AgentThread { .. } | ActiveView::TextThread { .. } => unreachable!(),
        }
    }
    ...
    fn thread_tab_title(view: &ActiveView, selected_agent: &AgentType, cx: &App) -> SharedString {
        match view {
            ActiveView::AgentThread { thread_view } => {
                Self::agent_thread_title_from_view(thread_view, selected_agent, cx)
            }
            ActiveView::TextThread {
                text_thread_editor, ..
            } => {
    ...
    fn thread_tab_status(view: &ActiveView, cx: &App) -> ThreadViewStatus {
        match view {
            ActiveView::AgentThread { thread_view } => {
                thread_view.read(cx).status_for_collaboration(cx)
            }
            ActiveView::Uninitialized
            | ActiveView::TextThread { .. }
            | ActiveView::History { .. }
    ...
    pub fn collaboration_presences(&self, cx: &App) -> Vec<AgentCollaborationPresence> {
        ...
        if let Some(current_view) = self.current_thread_view_for_tabs()
            && let ActiveView::AgentThread { thread_view } = current_view
        {
            add_presence(thread_view, &self.selected_agent);
        }

        for tab in &self.inactive_thread_tabs {
            if let ActiveView::AgentThread { thread_view } = &tab.view {
                add_presence(thread_view, &tab.selected_agent);
            }
        }
    ...
    fn take_current_thread_view(&mut self) -> Option<ActiveView> {
        ...
        if matches!(
            ...
            && Self::is_thread_view(&previous_view)
        {
            self.active_view = ActiveView::Uninitialized;
            return Some(previous_view);
        }

        None
    }

    fn begin_new_thread_tab(&mut self) {
        if let Some(current_view) = self.take_current_thread_view() {
            self.inactive_thread_tabs.push(InactiveThreadTab {
                id: self.active_thread_tab_id,
                view: current_view,
                selected_agent: self.selected_agent.clone(),
            });
            self.active_thread_tab_id = self.next_thread_tab_id;
            self.next_thread_tab_id += 1;
        }
    }

    fn activate_thread_tab(&mut self, tab_id: usize, window: &mut Window, cx: &mut Context<Self>) {
        if tab_id == self.active_thread_tab_id {
            return;
        }
        ...
        let tab = self.inactive_thread_tabs.remove(index);
        ...
        self.selected_agent = tab.selected_agent;
        self.active_thread_tab_id = tab.id;
        self.set_active_view(tab.view, true, window, cx);
        self.serialize(cx);
    }

    fn close_thread_tab(&mut self, tab_id: usize, window: &mut Window, cx: &mut Context<Self>) {
        if tab_id != self.active_thread_tab_id {
        ...
        let next_tab_index = self
            ...
            .iter()
            .enumerate()
            .max_by_key(|(_, tab)| tab.id)
            .map(|(index, _)| index);

        let Some(next_tab_index) = next_tab_index else {
            self.previous_view = None;
        ...
        let next_tab = self.inactive_thread_tabs.remove(next_tab_index);
        self.selected_agent = next_tab.selected_agent;
        self.active_thread_tab_id = next_tab.id;
        self.set_active_view(next_tab.view, true, window, cx);
        self.serialize(cx);
    }

    fn new_thread(&mut self, _action: &NewThread, window: &mut Window, cx: &mut Context<Self>) {
        if let Some(agent) = self.preferred_agent_for_new_thread(cx) {
    ...
    fn new_native_agent_thread_from_summary(
        ...
        if !AgentSettings::get_global(cx).show_zed_agent {
            self.open_configuration(window, cx);
            return;
        }
    ...
    fn external_thread_with_tab_mode(
        ...
        cx.spawn_in(window, async move |this, cx| {
            let requested_external_agent = match agent_choice {
                Some(agent) => {
                    cx.background_spawn({
            ...
            this.update_in(cx, |agent_panel, window, cx| {
                let Some(selected_external_agent) =
                    agent_panel.visible_external_agent_or_fallback(requested_external_agent, cx)
                else {
                    agent_panel.open_configuration(window, cx);
                    return;
                };

                let server = selected_external_agent.server(fs.clone(), thread_store.clone());
    ...
    fn history_kind_for_selected_agent(&self, cx: &App) -> Option<HistoryKind> {
        match self.selected_agent_for_display(cx) {
    ...
    fn open_active_thread_in_editor_tab(
        &mut self,
        _: &OpenActiveThreadInEditorTab,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    fn is_builtin_agent_visible(&self, agent_type: &AgentType, cx: &App) -> bool {
    ...
    fn custom_agent_exists(&self, custom_agent_name: &SharedString, cx: &App) -> bool {
        let agent_server_store = self.project.read(cx).agent_server_store().clone();
        agent_server_store
            .read(cx)
            .external_agents()
            .any(|agent_name| agent_name.0.as_ref() == custom_agent_name.as_ref())
    }

    fn is_agent_visible(&self, agent_type: &AgentType, cx: &App) -> bool {
        match agent_type {
            AgentType::Custom { name } => self.custom_agent_exists(name, cx),
            _ => self.is_builtin_agent_visible(agent_type, cx),
        }
    }

    fn first_visible_custom_agent(&self, cx: &App) -> Option<AgentType> {
        let agent_server_store = self.project.read(cx).agent_server_store().clone();
        agent_server_store
            .read(cx)
            .external_agents()
            ...
            .next()
            .map(|agent_name| AgentType::Custom {
                name: agent_name.0.clone(),
            })
    }

    fn first_visible_agent(&self, cx: &App) -> Option<AgentType> {
        let settings = AgentSettings::get_global(cx);
        if settings.show_zed_agent {
        ...
        if settings.show_gemini_cli {
            return Some(AgentType::Gemini);
        }
        self.first_visible_custom_agent(cx)
    }

    fn preferred_agent_for_new_thread(&self, cx: &App) -> Option<AgentType> {
        if self.is_agent_visible(&self.selected_agent, cx) {
            return Some(self.selected_agent.clone());
        }
        self.first_visible_agent(cx)
    }

    fn ensure_selected_agent_is_visible(&mut self, cx: &mut Context<Self>) {
        if self.is_agent_visible(&self.selected_agent, cx) {
            return;
        }

        let Some(first_visible_agent) = self.first_visible_agent(cx) else {
            return;
        };

        if self.selected_agent != first_visible_agent {
            self.selected_agent = first_visible_agent;
            self.serialize(cx);
        }
    }

    fn selected_agent_for_display(&self, cx: &App) -> AgentType {
        if self.is_agent_visible(&self.selected_agent, cx) {
            return self.selected_agent.clone();
        }
    ...
    fn selected_external_agent(&self, cx: &App) -> Option<ExternalAgent> {
        match &self.selected_agent_for_display(cx) {
    ...
    fn is_external_agent_visible(&self, external_agent: &ExternalAgent, cx: &App) -> bool {
        let settings = AgentSettings::get_global(cx);
        match external_agent {
            ExternalAgent::NativeAgent => settings.show_zed_agent,
            ExternalAgent::Gemini => settings.show_gemini_cli,
            ExternalAgent::ClaudeCode => settings.show_claude_code,
            ExternalAgent::Codex => settings.show_codex_cli,
            ExternalAgent::Custom { name } => self.custom_agent_exists(name, cx),
        }
    }

    fn first_visible_external_agent(&self, cx: &App) -> Option<ExternalAgent> {
        let settings = AgentSettings::get_global(cx);
        if settings.show_zed_agent {
            return Some(ExternalAgent::NativeAgent);
        }
        if settings.show_claude_code {
            return Some(ExternalAgent::ClaudeCode);
        }
        if settings.show_codex_cli {
            return Some(ExternalAgent::Codex);
        }
        if settings.show_gemini_cli {
            return Some(ExternalAgent::Gemini);
        }

        self.first_visible_custom_agent(cx).and_then(|agent_type| {
    ...
    fn visible_external_agent_or_fallback(
        &self,
        requested_external_agent: ExternalAgent,
        cx: &App,
    ) -> Option<ExternalAgent> {
        if self.is_external_agent_visible(&requested_external_agent, cx) {
            return Some(requested_external_agent);
        }
        self.first_visible_external_agent(cx)
    }
    ...
    fn sync_agent_servers_from_extensions(&mut self, cx: &mut Context<Self>) {
        ...
        self.ensure_selected_agent_is_visible(cx);
    }
    ...
    pub fn new_agent_thread(
        ...
        if !self.is_agent_visible(&agent, cx) {
            self.open_configuration(window, cx);
            return;
        }
...
impl AgentPanel {
    ...
    fn render_toolbar(&self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let agent_server_store = self.project.read(cx).agent_server_store().clone();
        let focus_handle = self.focus_handle(cx);
        let selected_agent_for_display = self.selected_agent_for_display(cx);

        let (selected_agent_custom_icon, selected_agent_label) =
            if let AgentType::Custom { name, .. } = &selected_agent_for_display {
                let store = agent_server_store.read(cx);
                let icon = store.agent_icon(&ExternalAgentServerName(name.clone()));

                let label = store
                    .agent_display_name(&ExternalAgentServerName(name.clone()))
                    .unwrap_or_else(|| selected_agent_for_display.label());
                (icon, label)
            } else {
                (None, selected_agent_for_display.label())
            };
        ...
        let new_thread_menu = PopoverMenu::new("new_thread_menu")
            ...
            .menu({
                let selected_agent = selected_agent_for_display.clone();
                ...
                move |window, cx| {
                    ...
                    Some(ContextMenu::build(window, cx, |menu, _window, cx| {
                        ...
                        let settings = AgentSettings::get_global(cx);
                        let show_zed_agent = settings.show_zed_agent;
                        let show_claude_code = settings.show_claude_code;
                        let show_codex_cli = settings.show_codex_cli;
                        let show_gemini_cli = settings.show_gemini_cli;
                        ...
                        let custom_agents = {
                            ...
                            agent_server_store
                                ...
                                .map(|agent_name| {
                                    ...
                                    (agent_name.clone(), icon_path, display_name)
                                })
                                .collect::<Vec<_>>()
                        };
                        ...
                        let show_external_agents = show_claude_code
                            || show_codex_cli
                            || show_gemini_cli
                            || !custom_agents.is_empty();

                        if show_external_agents {
                            if show_zed_agent {
                                menu = menu.separator();
                            }
                        ...
                        if show_zed_agent || show_external_agents {
                            menu = menu.separator();
                        }
        ...
        let selected_agent = div()
            ...
            .when(!has_custom_icon, |this| {
                this.when_some(selected_agent_for_display.icon(), |this, icon| {
    ...
    fn render_thread_tabs(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
        ...
        let selected_index = entries
            .iter()
            .position(|entry| entry.id == self.active_thread_tab_id)?;
        let total_entries = entries.len();

        let mut tab_bar = TabBar::new("agent-panel-thread-tabs");
            ...
            let close_button =
                IconButton::new(("close-agent-thread-tab", tab_id as u64), IconName::Close)
                    ...
                    .on_click(cx.listener(move |this, _, window, cx| {
                        cx.stop_propagation();
                        this.close_thread_tab(tab_id, window, cx);
                    }));

            let tab_position = if index == 0 {
                TabPosition::First
            } else if index == selected_index {
                TabPosition::Middle(Ordering::Equal)
            } else if index == total_entries.saturating_sub(1) {
                TabPosition::Last
            } else {
...
impl Render for AgentPanel {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let content = v_flex()
            ...
            .on_action(cx.listener(Self::open_active_thread_in_editor_tab))
...
impl EventEmitter<()> for AgentThreadEditorTabItem {}

impl Focusable for AgentThreadEditorTabItem {
    fn focus_handle(&self, cx: &App) -> FocusHandle {
        self.thread_view.focus_handle(cx)
    }
}
...
impl Render for AgentThreadEditorTabItem {
...
mod tests {
    ...
    async fn test_reopening_inactive_agent_session_focuses_existing_tab(cx: &mut TestAppContext) {
        ...
        panel.read_with(cx, |panel, cx| {
            ...
            let ActiveView::AgentThread { thread_view } = &panel.inactive_thread_tabs[0].view
            else {
                panic!("expected the remaining inactive tab to be an agent thread");
            };
            let inactive_session_id = thread_view
                .read(cx)
                .active_thread()
