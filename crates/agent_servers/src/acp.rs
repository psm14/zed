    ...
    fn apply_session_defaults(
        ...
        if let Some(config_options) = config_options.as_ref() {
            let defaults_to_apply: Vec<_> = {
                let config_options_ref = config_options.borrow();
                config_options_ref
                    .iter()
                        ...
                        let is_valid = match &config_option.kind {
                            acp::SessionConfigKind::Select(select) => match &select.options {
                                acp::SessionConfigSelectOptions::Ungrouped(options) => options
                                    .iter()
                                    .any(|option| &*option.value.0 == default_value.as_str()),
                                acp::SessionConfigSelectOptions::Grouped(groups) => {
                                    groups.iter().any(|group| {
                                        group.options.iter().any(|option| {
                                            &*option.value.0 == default_value.as_str()
                                        })
                                    })
                                }
                                _ => false,
            ...
            for (config_id, default_value, initial_value) in defaults_to_apply {
                cx.spawn({
                    ...
                    async move |_| {
                        ...
                        {
                            let mut options = config_options.borrow_mut();
                            if let Some(option) = options
                                .iter_mut()
                                .find(|option| option.id == config_id_clone)
...
impl AgentConnection for AcpConnection {
    ...
    fn new_session(
        ...
        cx.spawn(async move |cx| {
            ...
            self.apply_session_defaults(&response.session_id, &modes, &models, &config_options, cx);
    ...
    fn load_session(
        ...
        cx.spawn(async move |cx| {
            ...
            let (modes, models, config_options) =
                config_state(response.modes, response.models, response.config_options);
            self.apply_session_defaults(&session.session_id, &modes, &models, &config_options, cx);
