...
1. Agent follow primitives use `CollaboratorId::Agent` with session-targeted follow state (`agent_session_id`).
2. Agent location updates are session-scoped and drive follow behavior for the selected session.
...
2. Single click toggles follow/unfollow for that session.
...
7. Row click follows that row's session location.
...
5. Workspace follow state for `CollaboratorId::Agent` and selected `agent_session_id`.
...
2. Follow/unfollow from collaboration surfaces is session-targeted and matches existing follow semantics.
...
2. Should local-only (no room) mode prioritize active session ordering differently from room mode?
  - Current behavior is "active sessions/tabs with status."
